...
def set_execution_config(
    ...
    Args:
        ...
        flight_shuffle_shared_durability: How shared-directory writes are fsynced. For writes without EIO, "background" (the default) publishes the file immediately and fsyncs off the critical path; "none" skips fsync, so a shared copy can be lost if its writer node dies; "sync" fsyncs before publishing, which is the strongest but can cut write throughput several-fold on filesystems with expensive fsync. Recovered writes require a same-descriptor data sync before publication in every mode.
        flight_shuffle_eio_local_max_retries: In-process shuffle file EIO retries, including recovery read/write passes. Recovered writes require a same-descriptor data sync before publication; sync failures use task fallback. Defaults to 6; 0 disables local recovery. Budgets cover a read handle (including open), or each create/write phase. Does not re-execute operators.
        ...
        flight_shuffle_eio_max_retries: Maximum cumulative failed-task count allowing another attempt after shuffle file EIO. Defaults to 3; 0 disables retries. Earlier transient/worker failures consume this budget too; it is not an independent EIO counter. Requires restartable tasks without UDFs or external writes. File inputs must remain unchanged during execution.
